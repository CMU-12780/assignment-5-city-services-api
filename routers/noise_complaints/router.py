...
def list_noise_complaints(
    ...
    condition: Optional[ComplaintStatus] = Query(None, description="Filter by status"),
...
def delete_resource(noise_complaint_id: int, db: Session = Depends(get_db)):
    # DELETE resource
    deleted = crud.delete_noise_complaint(db=db, bridge_id=noise_complaint_id)
    if not deleted:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Bridge with id {noise_complaint_id} not found"
        )
    return None
    pass